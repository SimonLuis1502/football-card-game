...
export const createNewCard = (id, name, club, trophies, games, goals) => {
    try {if (isNaN(id)){
        throw new Error(`${id} is not a number`);
    }
    if (!isNaN(club)){
        throw new Error(`${club} is not a valid club`);
    }
    if (isNaN(trophies)){
        throw new Error(`${trophies} is not a number`);
    }
    return new Card(id, name, club, trophies, games, goals);
    }catch(error){
        console.log(error);
    }
}
